...
public class KafkaListenerController {
    ...
    public Message<ResponseBody> handleMessage(RequestBody requestBody,
        ...
        switch (requestType) {
            ...
            case START_ARCHIVE_PIPELINE:
                try {
                    ...
                    pipelineService.startPipelineJob(requestBody.getPipelineName(), requestBody.getBranch(),
                          requestBody.getProfileName(), requestBody.getJobs());
                    final ResponseBody responseBody = ResponseBody.builder().build();
                    return buildResponseMessage(responseBody, ResponseType.START_ARCHIVE_PIPELINE_RESPONSE, replyTo,
                          correlationId);
                } catch (Exception e) {
            ...
            default:
                String errorMessage = String.format("Unknown request type %s", requestType);
                log.error(errorMessage);
                return handleErrorResponse(new RuntimeException(errorMessage), replyTo, correlationId);
        }
        return null;
    }
