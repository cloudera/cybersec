...
public class ParserWriterConfiguration implements WriterConfiguration {
    private final ParserConfigurations config;
    ...
    public int getBatchSize(String sensorName) {
        if (config != null
            ...
            Object batchObj = config.getSensorParserConfig(sensorName).getParserConfig()
                                    .get(IndexingConfigurations.BATCH_SIZE_CONF);
    ...
    public int getBatchTimeout(String sensorName) {
        if (config != null
            ...
            Object batchObj = config.getSensorParserConfig(sensorName).getParserConfig()
                                    .get(IndexingConfigurations.BATCH_TIMEOUT_CONF);
    ...
    public String getIndex(String sensorName) {
        if (config != null && config.getSensorParserConfig(sensorName) != null
            ...
            Object indexObj =
                  config.getSensorParserConfig(sensorName).getParserConfig().get(IndexingConfigurations.INDEX_CONF);
            if (indexObj != null) {
    ...
    public boolean isEnabled(String sensorName) {
        if (config != null
            ...
            Object enabledObj =
                  config.getSensorParserConfig(sensorName).getParserConfig().get(IndexingConfigurations.ENABLED_CONF);
            return enabledObj == null || ConversionUtils.convert(enabledObj, Boolean.class);
        }
        return true;
    }
