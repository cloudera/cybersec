...
public enum FieldValidations {
    STELLAR(new QueryValidation()), IP(new IPValidation()), DOMAIN(new DomainValidation()),
    EMAIL(new EmailValidation()), URL(new URLValidation()), DATE(new DateValidation()),
    INTEGER(new IntegerValidation()), REGEX_MATCH(new RegexValidation()), NOT_EMPTY(new NotEmptyValidation());
