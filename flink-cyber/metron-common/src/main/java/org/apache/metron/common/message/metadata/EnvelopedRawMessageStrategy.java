...
public class EnvelopedRawMessageStrategy implements RawMessageStrategy {
     ...
     */
    @Override
    public RawMessage get(Map<String, Object> rawMetadata, byte[] rawMessage, boolean readMetadata,
        ...
        try {
            ...
            if (message != null) {
                if (!readMetadata) {
                    LOG.debug(
                          "Ignoring metadata; Message: " + message + " rawMetadata: " + rawMetadata + " and field = "
                          + messageField);
                    ...
                    LOG.debug(
                          "Attaching metadata; Message: " + message + " rawMetadata: " + rawMetadata + " and field = "
                          + messageField);
    ...
    public void mergeMetadata(JSONObject message, Map<String, Object> metadata, boolean mergeMetadata,
        ...
        String prefix = MetadataUtil.INSTANCE.getMetadataPrefix(config);
        String originalStringFromMetadata =
              (String) metadata.get(MetadataUtil.INSTANCE.prefixKey(prefix, Constants.Fields.ORIGINAL.getName()));
        if (mergeMetadata) {
