...
public class StatsProfileGroupAcc extends ProfileGroupAcc {
    ...
    public static final List<String> STATS_EXTENSION_SUFFIXES =
          Lists.newArrayList(MIN_RESULT_SUFFIX, MAX_RESULT_SUFFIX, MEAN_RESULT_SUFFIX, STDDEV_RESULT_SUFFIX);
    ...
    protected void updateAccumulators(ProfileMessage message, ProfileGroupConfig profileGroupConfig) {
        Iterator<ProfileMeasurementConfig> measurementIter = profileGroupConfig.getMeasurements().stream()
                                                                               .filter(
                                                                                     ProfileMeasurementConfig::hasStats)
                                                                               .iterator();
    ...
    protected void addExtensions(ProfileGroupConfig profileGroupConfig, Map<String, String> extensions,
                                 Map<String, DecimalFormat> ignored) {
        Iterator<ProfileMeasurementConfig> measurementIter = profileGroupConfig.getMeasurements().stream()
                                                                               .filter(
                                                                                     ProfileMeasurementConfig::hasStats)
                                                                               .iterator();
        ...
        while (measurementIter.hasNext() && accumulatorIter.hasNext()) {
            ...
            extensions.put(resultExtensionName.concat(STDDEV_RESULT_SUFFIX),
                  DEFAULT_FORMAT.format(stats.getStandardDeviation()));
        }
    }
}
