 ...
 * Otherwise, if one of the profile measurement is cyber score details or does not need a field, return the message.
 * The cyber score details field is always guaranteed to be specified.
 */
public class ProfileMessageFilter implements FilterFunction<ScoredMessage> {
    ...
    public ProfileMessageFilter(ProfileGroupConfig profileGroupConfig) {
        ...
        this.keyFieldNames = profileGroupConfig.getKeyFieldNames().stream().filter(
                                                     fieldName -> !ScoredMessageToProfileMessageMap.CYBER_SCORE_FIELD.equals(fieldName))
                                               .collect(Collectors.toList());
        List<String> optionalFields = profileGroupConfig.getMeasurementFieldNames().stream()
                                                        .filter(
                                                              fieldName -> !ScoredMessageToProfileMessageMap.CYBER_SCORE_FIELD.equals(
                                                                    fieldName))
                                                        .collect(Collectors.toList());
