...
public class StellarCompiler extends StellarBaseListener {
    private static final Token<?> EXPRESSION_REFERENCE = new Token<>(null, Object.class);
    private static final Token<?> LAMBDA_VARIABLES = new Token<>(null, Object.class);
        ...
        public Object apply(ExpressionState state) {
            Deque<Token<?>> instanceDeque = new ArrayDeque<>();
            {
                ...
                for (Iterator<Token<?>> it = getTokenDeque().descendingIterator(); it.hasNext(); ) {
                    ...
                    if (isFalsey) {
                        //If we're in a situation where the token is a boolean token
                        // and the current value is one of the implicitly falsey scenarios
    ...
    public void exitArithExpr_plus(StellarParser.ArithExpr_plusContext ctx) {
        ...
        expression.tokenDeque.push(new Token<>((tokenDeque, state) -> {
            ...
            tokenDeque.push(
                  arithmeticEvaluator.evaluate(ArithmeticEvaluator.ArithmeticEvaluatorFunctions.addition(context), p));
    ...
    public void exitArithExpr_minus(StellarParser.ArithExpr_minusContext ctx) {
        ...
        expression.tokenDeque.push(new Token<>((tokenDeque, state) -> {
            Pair<Token<? extends Number>, Token<? extends Number>> p = getArithExpressionPair(tokenDeque);
            tokenDeque.push(
                  arithmeticEvaluator.evaluate(ArithmeticEvaluator.ArithmeticEvaluatorFunctions.subtraction(context),
                        p));
        }, DeferredFunction.class, context));
    }
    ...
    public void exitArithExpr_div(StellarParser.ArithExpr_divContext ctx) {
        ...
        expression.tokenDeque.push(new Token<>((tokenDeque, state) -> {
            Pair<Token<? extends Number>, Token<? extends Number>> p = getArithExpressionPair(tokenDeque);
            tokenDeque.push(
                  arithmeticEvaluator.evaluate(ArithmeticEvaluator.ArithmeticEvaluatorFunctions.division(context), p));
        }, DeferredFunction.class, context));
    }
    ...
    public void exitArithExpr_mul(StellarParser.ArithExpr_mulContext ctx) {
        ...
        expression.tokenDeque.push(new Token<>((tokenDeque, state) -> {
            Pair<Token<? extends Number>, Token<? extends Number>> p = getArithExpressionPair(tokenDeque);
            tokenDeque.push(
                  arithmeticEvaluator.evaluate(ArithmeticEvaluator.ArithmeticEvaluatorFunctions.multiplication(context),
                        p));
        }, DeferredFunction.class, context));
    }
    ...
    public void exitMatchClauseCheckExpr(StellarParser.MatchClauseCheckExprContext ctx) {
        ...
        expression.tokenDeque.push(
              new Token<>(new MatchClauseCheckExpr(), MatchClauseCheckExpr.class, getArgContext()));
    }
