...
public class ClusterService {
    ...
    public List<Pipeline> getClusterPipelines(ResponseBody responseBody) {
        ...
        for (Job job : responseBody.getJobs()) {
            Optional<Pipeline> duplicatePipeline = result.stream().filter(pipeline ->
                  StringUtils.equalsIgnoreCase(pipeline.getName(), job.getJobPipeline())).findFirst();
    ...
    public ResponseBody startPipelineJob(String clusterId, String pipeline, String branch, String profileName,
                                         String parserName, List<String> jobs, byte[] payload)
          throws FailedClusterReponseException {
        Pair<ResponseType, ResponseBody> response =
              kafkaService.sendWithReply(RequestType.START_PIPELINE, clusterId, RequestBody
                    .builder()
