...
public class KafkaService {
    ...
    public KafkaService(@Qualifier("kafkaTemplatePool")
    ...
    public Pair<ResponseType, ResponseBody> sendWithReply(RequestType requestType, String clusterId, RequestBody body) {
        final ClouderaReplyingKafkaTemplate<String, RequestBody, ResponseBody> kafkaTemplate =
              kafkaTemplatePool.get(clusterId);
    ...
    private Pair<ResponseType, ResponseBody> send(RequestType requestType, RequestBody body, String clusterId,
                                                  ClouderaReplyingKafkaTemplate<String, RequestBody, ResponseBody> kafkaTemplate) {
        ProducerRecord<String, RequestBody> producerRecord =
              new ProducerRecord<>(kafkaTemplate.getRequestTopic(), requestType.name(), body);
        RequestReplyFuture<String, RequestBody, ResponseBody> replyFuture =
              kafkaTemplate.sendAndReceive(producerRecord, Duration.ofSeconds(kafkaTemplateTimeout));
        try {
            ...
            return Pair.of(Utils.getEnumFromString(consumerRecord.key(), ResponseType.class, ResponseType::name),
                  responseBody);
        } catch (ExecutionException e) {
            // Handle exception
            log.error(
                  "Exception thrown when attempting to retrieve the information from kafka. Message: '{}' \n Cause: '{}'",
                  e.getMessage(), e.getCause().getMessage());
            return Pair.of(null, ResponseBody.builder().clusterMeta(
                  ClusterMeta.builder().clusterId(clusterId).clusterStatus("offline").build()).build());
