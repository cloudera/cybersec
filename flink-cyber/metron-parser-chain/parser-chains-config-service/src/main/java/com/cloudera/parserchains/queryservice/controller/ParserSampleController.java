...
public class ParserSampleController {
    ...
    public ResponseEntity<List<ParserSample>> findAllById(
          @Parameter(name = "id", description = "The ID of the parser chain to retrieve samples for.", required = true)
    ...
    @Operation(summary = "Create or replace parser chain sample list.",
          responses = {
                @ApiResponse(responseCode = "204", description = "The parser chain list was created/replaced.",
                      content = @Content(array = @ArraySchema(schema = @Schema(implementation = ParserSample.class)))),
    ...
    public ResponseEntity<List<ParserSample>> update(
        ...
        String sampleFolderPath = getSampleFolderPath(body);
        try {
            List<ParserSample> createdSampleList =
                  parserSampleService.update(sampleFolderPath, id, body.getSampleList());
