...
public class ParserSampleController {
    ...
    @ApiResponses(value = {@ApiResponse(code = 200, message = "A list of all parser samples for the specified chain.")})
    @PostMapping(value = API_PARSER_TEST_SAMPLES + "/{id}")
    public ResponseEntity<List<ParserSample>> findAllById(@ApiParam(name = "id", value = "The ID of the parser chain to retrieve samples for.", required = true) @PathVariable String id, @RequestBody SampleFolderDescriptor body) throws IOException {
    ...
    @ApiResponses(value = {@ApiResponse(code = 204, message = "The parser chain list was created/replaced."), @ApiResponse(code = 404, message = "The parser chain does not exist.")})
    @PutMapping(value = API_PARSER_TEST_SAMPLES + "/{id}")
    public ResponseEntity<List<ParserSample>> update(@ApiParam(name = "sampleList", value = "The new sample definition list.", required = true) @RequestBody SampleFolderDescriptor body, @ApiParam(name = "id", value = "The ID of the parser chain sample to update.") @PathVariable String id) {
        String sampleFolderPath = getSampleFolderPath(body);
        try {
            ...
            return ResponseEntity.created(URI.create(API_PARSER_TEST_SAMPLES + "/" + id)).body(createdSampleList);
    ...
    private String getSampleFolderPath(SampleFolderDescriptor body) {
        String sampleFolderPath = StringUtils.hasText(body.getFolderPath()) ? body.getFolderPath() : appProperties.getSampleFolderPath();
