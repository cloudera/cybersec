...
public class MockKafkaService implements KafkaServiceInterface {
    ...
    private final List<ResponseBody> responses =
          Lists.newArrayList(responseFromCluster1, responseFromCluster2, responseFromCluster3);

    @Override
    public Pair<ResponseType, ResponseBody> sendWithReply(RequestType requestType, String clusterId, RequestBody body) {
        return responses.stream()
                        .filter(
                              response -> StringUtils.equalsIgnoreCase(response.getClusterMeta().getName(), clusterId))
