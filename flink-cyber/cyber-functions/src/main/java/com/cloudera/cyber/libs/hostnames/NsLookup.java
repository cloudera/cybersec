                         ...
                         try {
                             ...
                             if (lookup.getResult() == Lookup.SUCCESSFUL) {
                                 if (records != null) {
                                     ...
                                     log.warn("Dns Lookup query='{}' type='{}' returned null records", key.getQuery(),
                                           key.getType());
                                     return null;
                                 }
                             } else {
                                 log.error("Dns Lookup query='{}' type='{}' failed with error {}", key.getQuery(),
                                       key.getType(), lookup.getErrorString());
                                 return null;
                             }
                         } catch (TextParseException e) {
                             log.error(String.format("Dns Lookup query='%s' type='%s' failed.", key.getQuery(), key.getType()),
                                   e);
                             return null;
                         }
                     }
