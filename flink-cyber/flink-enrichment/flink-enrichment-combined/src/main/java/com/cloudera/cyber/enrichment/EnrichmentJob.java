...
public abstract class EnrichmentJob {
    ...
    protected StreamExecutionEnvironment createPipeline(ParameterTool params) throws IOException {
        ...
        List<EnrichmentConfig> enrichmentConfigs =
              ConfigUtils.allConfigs(Files.readAllBytes(Paths.get(params.getRequired(PARAMS_LOOKUPS_CONFIG_FILE))));
        SingleOutputStreamOperator<EnrichmentCommand> localEnrichments =
              enrichments.filter(new FilterEnrichmentType(enrichmentConfigs, EnrichmentKind.LOCAL));
        SingleOutputStreamOperator<EnrichmentCommand> hbaseEnrichments =
              enrichments.filter(new FilterEnrichmentType(enrichmentConfigs, EnrichmentKind.HBASE));
        ...
        Tuple2<DataStream<Message>, DataStream<EnrichmentCommandResponse>> enriched =
              LookupJob.enrich(localEnrichments, cidrEnriched, enrichmentConfigs);
        ...
        if (threatqEnabled) {
            List<ThreatQConfig> threatQconfigs = ThreatQJob.parseConfigs(
                  Files.readAllBytes(Paths.get(params.getRequired(PARAMS_THREATQ_CONFIG_FILE))));
        ...
        if (hbaseEnabled || threatqEnabled) {
            hbaseTqEnrichResults =
                  writeHbaseThreatQEnrichmentsToHbaseAndRespond(params, env, hbaseEnrichments, threatqEnrichments,
                        enrichmentsStorageConfig);
        }
        ...
        if (params.getBoolean(PARAMS_ENABLE_STELLAR, true)) {
            ...
            stellarStream =
                  StellarEnrichmentJob.enrich(tqed, StellarEnrichmentJob.loadFiles(configDir), geoDatabasePath,
                        asnDatabasePath);
        } else {
            stellarStream = tqed;
        }
    ...
    private DataStream<EnrichmentCommandResponse> writeHbaseThreatQEnrichmentsToHbaseAndRespond(
          ParameterTool params,
          StreamExecutionEnvironment env,
          DataStream<EnrichmentCommand> hbaseEnrichments,
          DataStream<EnrichmentCommand> threatqEnrichments,
          EnrichmentsConfig enrichmentsStorageConfig) {

        // write the threatq and hbase enrichments to hbase
        return new HbaseJobRawKafka().writeEnrichments(env, params, unionStreams(hbaseEnrichments, threatqEnrichments),
              enrichmentsStorageConfig);
    }
    ...
    /**
     * Method to implement that's designed to apply rules.
     *
    ...
    private DataStream<ScoredMessage> doScoring(DataStream<Message> in, StreamExecutionEnvironment env,
                                                ParameterTool params) {
    ...
    protected abstract void writeResults(StreamExecutionEnvironment env, ParameterTool params,
                                         DataStream<ScoredMessage> results);

    protected abstract DataStream<EnrichmentCommand> createEnrichmentSource(StreamExecutionEnvironment env,
                                                                            ParameterTool params);

    protected abstract void writeEnrichmentQueryResults(StreamExecutionEnvironment env, ParameterTool params,
                                                        DataStream<EnrichmentCommandResponse> sideOutput);

    protected abstract DataStream<EnrichmentCommand> createThreatQSource(StreamExecutionEnvironment env,
                                                                         ParameterTool params);
