...
public abstract class HbaseJob {
    ...
    protected StreamExecutionEnvironment createPipeline(ParameterTool params) throws IOException {
        ...
        DataStream<EnrichmentCommandResponse> enrichmentCommandResponses =
              writeEnrichments(env, params, enrichmentSource, enrichmentsConfig);
        writeQueryResults(env, params, enrichmentCommandResponses);

        return env;
    }

    public abstract DataStream<EnrichmentCommandResponse> writeEnrichments(StreamExecutionEnvironment env,
