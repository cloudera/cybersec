...
public class IpRegionCidr {
    ...
    public static SingleOutputStreamOperator<Message> cidr(SingleOutputStreamOperator<Message> source,
                                                           List<String> ipFields, String confPath) throws IOException {
        RegionCidrEnrichmentConfiguration regionCidrEnrichmentConfiguration =
              JSONUtils.INSTANCE.load(readConfigFile(confPath), RegionCidrEnrichmentConfiguration.class);
    ...
    private static void validate(RegionCidrEnrichmentConfiguration regionCidrEnrichmentMap) {
        ...
        if (CollectionUtils.isNotEmpty(ValidateUtils.getDuplicates(regionNames))) {
            throw new ValidationException(String.format("Configuration contains duplicates in region names: '%s'",
                  String.join(",", regionNames)));
        }
        if (CollectionUtils.isNotEmpty(ValidateUtils.getDuplicates(cidrs))) {
            throw new ValidationException(
                  String.format("Configuration contains duplicates in cidrs: '%s'", String.join(",", cidrs)));
        }
        for (String cidr : cidrs) {
            try {
                IPAddress ipAddress = new IPAddressString(cidr).toAddress();
                if (ipAddress.getPrefixLength() == null
                        || ipAddress.getCount().equals(BigInteger.ONE) && ipAddress.getPrefixLength() != 32) {
                    throw new ValidationException(String.format(
                          "Wrong format for cidr: '%s'. Only the subnet is allowed to be used as Cidr values. "
                          + "1.2.0.0/16 - is the subnet; 1.2.3.4/16 is a single address, since the host is 3.4; "
                          + "1.2.3.4 is a single address, since have no prefix",
                          cidr));
                }
