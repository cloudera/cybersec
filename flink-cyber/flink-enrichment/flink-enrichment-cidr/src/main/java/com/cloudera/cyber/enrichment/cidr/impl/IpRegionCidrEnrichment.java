...
public class IpRegionCidrEnrichment {
    ...
    public static final String FEATURE_NAME = "region";


    public void lookup(String ipFieldName, String ipFieldValue, Map<String, String> extensions,
        ...
        if (ipAddressString.getAddress() == null) {
            MessageUtils.addQualityMessage(qualityMessages, DataQualityMessageLevel.ERROR,
                  "Unable to parse message with ip " + ipFieldValue, ipFieldName, FEATURE_NAME);
            log.error("Unable to parse ip in a message {}", extensions);
            return;
        }

        Optional<String> regionNameOptional = regionCidrMap.entrySet().stream()
                                                           .filter(entry -> entry.getKey().getAddress().toIPv6()
                                                                                 .contains(ipAddressString.getAddress()
                                                                                                          .toIPv6()))
