...
public abstract class BatchEnrichmentLoaderCSV extends BatchEnrichmentLoader {
    ...
    protected void load(StreamExecutionEnvironment env, ParameterTool params) {
        ...
        CsvReaderFormat<EnrichmentCommand> csvFormat =
              CsvToEnrichmentCommandDeserializer.createCsvReaderFormat(enrichmentConfig, columnNames, ignoreFirstLine,
                    enrichmentType);
        ...
        DataStream<EnrichmentCommand> csvEnrichment =
              env.fromSource(source, WatermarkStrategy.noWatermarks(), "Enrichment CSV");

        writeResults(params, enrichmentsConfig, enrichmentType, csvEnrichment, env);
    }

    private EnrichmentsConfig getEnrichmentsConfig(ParameterTool params, String enrichmentType,
        ...
        if (enrichmentConfigFile != null) {
            ...
            Preconditions.checkNotNull(enrichmentConfig,
                  String.format("No configuration for enrichmentType '%s' found in enrichments config file '%s'",
                        enrichmentType, enrichmentConfigFile));

            Preconditions.checkState(keyFieldName == null,
                  String.format("Key field name should not be specified when using %s configuration.",
                        PARAMS_ENRICHMENT_FILE));

            List<String> valueFields = enrichmentConfig.getFieldMapping().getValueFields();
            if (valueFields != null) {
                List<String> valueFieldsInColumns =
                      columnNames.stream().filter(valueFields::contains).collect(Collectors.toList());
                Preconditions.checkState(!valueFieldsInColumns.isEmpty(),
                      String.format("Columns specified in csv '%s' do not contain any value fields '%s'",
                            String.join(",", columnNames), String.join(",", valueFields)));
            }
            log.info("Using enrichment config file {}", enrichmentConfig);
        } else {
            Preconditions.checkNotNull(keyFieldName,
                  String.format("Missing key field configuration %s", ENRICHMENT_KEY_FIELD_NAME));
            String enrichmentsTable = params.getRequired(PARAMS_ENRICHMENTS_TABLE);
            enrichmentsConfig = new EnrichmentsConfig();
            enrichmentsConfig.getStorageConfigs().put(DEFAULT_ENRICHMENT_STORAGE_NAME,
                  new EnrichmentStorageConfig(HBASE_SIMPLE, enrichmentsTable, null));
            EnrichmentFieldsConfig fieldsConfig =
                  new EnrichmentFieldsConfig(Lists.newArrayList(keyFieldName), null, null, null);
        ...
        List<String> missingKeyFields =
              keyFields.stream().filter(kf -> !columnNames.contains(kf)).collect(Collectors.toList());
        Preconditions.checkState(missingKeyFields.isEmpty(),
              String.format("Columns '%s' are missing key field(s) '%s'", String.join(",", columnNames),
                    String.join(", ", missingKeyFields)));
