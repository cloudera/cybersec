...
public abstract class MaxMindBase {
    private static final Function<String, DatabaseProvider> MEMOIZER =
          Memoizer.memoize(MaxMindBase::loadDatabaseProvider);
    ...
    protected MaxMindBase(String geocodeDatabasePath) {
        Preconditions.checkArgument(StringUtils.isNotBlank(geocodeDatabasePath),
              "The path to Maxmind database is blank '%s'", geocodeDatabasePath);
    ...
    private static DatabaseReader createDatabaseReader(String geocodeDatabasePath, FileSystem fileSystem)
          throws IOException {
    ...
    protected InetAddress convertToIpAddress(Enrichment enrichment, Object ipValueObject,
        ...
        if (ipValueObject instanceof String) {
            ...
            if (InetAddressValidator.getInstance().isValid(ipValue)) {
                try {
                    ...
                    enrichment.addQualityMessage(qualityMessages, DataQualityMessageLevel.INFO,
                          String.format(MAXMIND_FAILED_MESSAGE, e.getMessage()));
                }
            } else {
                enrichment.addQualityMessage(qualityMessages, DataQualityMessageLevel.INFO,
                      String.format(FIELD_VALUE_IS_NOT_A_VALID_IP_ADDRESS, ipValue));
            }
        } else {
            enrichment.addQualityMessage(qualityMessages, DataQualityMessageLevel.INFO,
                  String.format(FIELD_VALUE_IS_NOT_A_STRING, ipValueObject.toString()));
        }
