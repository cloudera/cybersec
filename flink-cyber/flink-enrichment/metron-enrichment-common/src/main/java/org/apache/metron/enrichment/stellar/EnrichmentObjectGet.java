...
@Stellar(namespace = "ENRICHMENT",
      ...
      description = "Retrieve and deserialize a serialized object from HDFS and stores it in the ObjectCache,  "
                    ...
                    + "\"" + ObjectCacheConfig.OBJECT_CACHE_SIZE_KEY + "\" (default "
                    + ObjectCacheConfig.OBJECT_CACHE_SIZE_DEFAULT + "),"
                    + "\"" + ObjectCacheConfig.OBJECT_CACHE_EXPIRATION_KEY + "\" (default "
                    + ObjectCacheConfig.OBJECT_CACHE_EXPIRATION_MIN_DEFAULT + "),"
                    + "\"" + ObjectCacheConfig.OBJECT_CACHE_TIME_UNIT_KEY + "\" (default MINUTES)."
                    +
                    "Cache settings that apply only to this function can also be specified in the "
                    + "global config by nesting the settings above under the "
                    + ENRICHMENT_OBJECT_GET_SETTINGS + " key."
...
public class EnrichmentObjectGet implements StellarFunction {
    ...
    public static final String ENRICHMENT_OBJECT_GET_SETTINGS = "enrichment.object.get.settings";
    ...
    public Object apply(List<Object> args, Context context) throws ParseException {
        ...
        try {
            ...
            throw new ClassCastException(
                  String.format("The object stored in HDFS at '%s' must be serialized in JSON format.", path));
        }
    ...
    public void initialize(Context context) {
        Map<String, Object> config =
              (Map<String, Object>) context.getCapability(Context.Capabilities.GLOBAL_CONFIG, false)
                                           .orElse(new HashMap<>());
        Map<String, Object> enrichmentGetConfig =
              (Map<String, Object>) config.getOrDefault(ENRICHMENT_OBJECT_GET_SETTINGS, new HashMap<>());
