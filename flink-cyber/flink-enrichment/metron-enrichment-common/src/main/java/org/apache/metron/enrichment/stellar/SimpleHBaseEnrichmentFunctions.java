...
public class SimpleHBaseEnrichmentFunctions {
    ...
    private static class WrapperTable {
        ...
        public boolean equals(Object o) {
            ...
            if (!Objects.equals(name, table.name)) {
                return false;
            }
            return Objects.equals(columnFamily, table.columnFamily);
    ...
    private static synchronized void initializeTracker(Map<String, Object> config, TableProvider provider)
          throws IOException {
        if (tracker == null) {
            String accessTrackerType =
                  (String) config.getOrDefault(ACCESS_TRACKER_TYPE_CONF, AccessTrackers.NOOP.toString());
    ...
    private static TableProvider createProvider(String tableProviderClass) {
        try {
            Class<? extends TableProvider> providerClazz =
                  (Class<? extends TableProvider>) Class.forName(tableProviderClass);
    ...
    private static synchronized void initializeProvider(Map<String, Object> config) {
        if (provider == null) {
            String tableProviderClass =
                  (String) config.getOrDefault(TABLE_PROVIDER_TYPE_CONF, HTableProvider.class.getName());
    ...
    private static void initializeHbaseConfiguration(Map<String, Object> config) {
        if (hBaseConfiguration == null) {
            hBaseConfiguration = (Configuration) config.getOrDefault(HbaseConfiguration.HBASE_CONFIG_NAME,
    ...
    @Stellar(name = "EXISTS",
          ...
          description =
                "Interrogates the HBase table holding the simple hbase enrichment data and returns whether the"
    ...
    public static class EnrichmentExists implements StellarFunction {
        ...
        private static final Cache<WrapperTable, EnrichmentLookup> enrichmentCollateralCache = CacheBuilder.newBuilder()
                                                                                                           .build();
        ...
        public Object apply(List<Object> args, Context context) throws ParseException {
            ...
            if (args.size() != 4) {
                throw new IllegalStateException(
                      "All parameters are mandatory, submit 'enrichment type', 'indicator', 'nosql_table' and 'column_family'");
            }
            int i = 0;
            ...
            try {
                lookup = enrichmentCollateralCache.get(key, () -> {
                          Table hbaseTable = provider.getTable(hBaseConfiguration, key.name);
                          return new EnrichmentLookup(hbaseTable, key.columnFamily, tracker);
                      }
                );
    ...
    public static class EnrichmentGet implements StellarFunction {
        ...
        public Object apply(List<Object> args, Context context) throws ParseException {
            ...
            EnrichmentLookup.HBaseContext hbaseContext = new EnrichmentLookup.HBaseContext(lookup.getTable(), cf);
            try {
                LookupKV<EnrichmentKey, EnrichmentValue> kv =
                      lookup.get(new EnrichmentKey(enrichmentType, indicator), hbaseContext, true);
