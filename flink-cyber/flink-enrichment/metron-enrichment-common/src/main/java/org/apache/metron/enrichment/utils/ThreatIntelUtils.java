...
public class ThreatIntelUtils {
    ...
    public static JSONObject triage(JSONObject ret, SensorEnrichmentConfig config, FunctionResolver functionResolver,
        ...
        if (isAlert) {
            ...
            if (triageConfig != null) {
                ...
                if (LOG.isDebugEnabled()
                        && (triageConfig.getRiskLevelRules() == null || triageConfig.getRiskLevelRules().isEmpty())) {
    ...
    private static void appendThreatScore(ThreatScore threatScore, JSONObject message) {
        ...
        for (RuleScore score : threatScore.getRuleScores()) {
            ...
            message.put(joiner.join(THREAT_TRIAGE_RULES_KEY, i, THREAT_TRIAGE_RULE_COMMENT),
                  score.getRule().getComment());
            message.put(joiner.join(THREAT_TRIAGE_RULES_KEY, i, THREAT_TRIAGE_RULE_SCORE),
                  score.getRule().getScoreExpression());
