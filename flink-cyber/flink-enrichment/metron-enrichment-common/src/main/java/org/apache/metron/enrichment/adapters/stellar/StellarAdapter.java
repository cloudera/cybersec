...
public class StellarAdapter implements EnrichmentAdapter<CacheKey>, Serializable {
            ...
            for (Map.Entry<String, Object> kv : stellarStatements) {
                if (kv.getKey() != null && kv.getValue() != null) {
                    if (kv.getValue() instanceof String) {
                        ...
                        Object o = null;
                        try {
                            o = processor.parse(stellarStatement, resolver, StellarFunctions.FUNCTION_RESOLVER(),
                                  stellarContext);
                        } catch (Exception e) {
    ...
    public JSONObject enrich(CacheKey value) {
        ...
        if (_PERF_LOG.isDebugEnabled()) {
            slowLogThreshold =
                  ConversionUtils.convert(globalConfig.getOrDefault(STELLAR_SLOW_LOG, STELLAR_SLOW_LOG_DEFAULT),
                        Long.class);
        }
