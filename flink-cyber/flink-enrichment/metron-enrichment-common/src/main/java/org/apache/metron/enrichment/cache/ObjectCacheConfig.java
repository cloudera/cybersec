...
public class ObjectCacheConfig {
    ...
    public ObjectCacheConfig(Map<String, Object> config) {
        cacheSize = ConversionUtils.convert(config.getOrDefault(OBJECT_CACHE_SIZE_KEY, OBJECT_CACHE_SIZE_DEFAULT),
              Long.class);
        if (config.containsKey(OBJECT_CACHE_EXPIRATION_MINUTES_KEY)) {
            cacheExpiration = ConversionUtils.convert(
                  config.getOrDefault(OBJECT_CACHE_EXPIRATION_MINUTES_KEY, OBJECT_CACHE_EXPIRATION_MIN_DEFAULT),
                  Long.class);
            timeUnit = OBJECT_CACHE_TIME_UNIT_DEFAULT;
        } else {
            cacheExpiration = ConversionUtils.convert(
                  config.getOrDefault(OBJECT_CACHE_EXPIRATION_KEY, OBJECT_CACHE_EXPIRATION_MIN_DEFAULT), Long.class);
        ...
        maxFileSize = ConversionUtils.convert(
              config.getOrDefault(OBJECT_CACHE_MAX_FILE_SIZE_KEY, OBJECT_CACHE_MAX_FILE_SIZE_DEFAULT), Long.class);
    }
