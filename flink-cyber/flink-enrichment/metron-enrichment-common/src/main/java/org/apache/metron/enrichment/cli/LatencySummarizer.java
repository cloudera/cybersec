...
public class LatencySummarizer {
    ...
    public static class LatencyStats {
        private final NavigableMap<Integer, Map<Pair, DescriptiveStatistics>> depthMap = new TreeMap<>();
        ...
        public void printDepthSummary(int depth, boolean meanOnly) {
            ...
            Collections.sort(sortedStats, new Comparator<Map.Entry<Pair, DescriptiveStatistics>>() {
                @Override
                public int compare(Map.Entry<Pair, DescriptiveStatistics> o1,
                                   Map.Entry<Pair, DescriptiveStatistics> o2) {
            ...
            for (Map.Entry<Pair, DescriptiveStatistics> stats : sortedStats) {
                summary(stats.getKey().getKey() + " -> " + stats.getKey().getValue(), stats.getValue(), System.out,
                      meanOnly);
            }
        }
