...
public class HTableProvider implements TableProvider {
     ...
     */
    private static final Map<Configuration, ThreadLocal<RetryingConnection>> connMap = new ConcurrentHashMap<>();
    ...
    private Connection getConnection(Configuration config) throws IOException {
        ThreadLocal<RetryingConnection> threadLocal =
              connMap.computeIfAbsent(config, c -> ThreadLocal.withInitial(() -> new RetryingConnection(config)));
