...
public class BasicPaloAltoFirewallParser extends BasicParser {
    ...
    private void parseMessage(String message, JSONObject outputMessage) {
        ...
        if (!empty_attribute(tokens[0])) {
            outputMessage.put(PaloAltoDomain, tokens[0].trim());
        }
        if (!empty_attribute(tokens[1])) {
            outputMessage.put(ReceiveTime, tokens[1].trim());
        }
        if (!empty_attribute(tokens[2])) {
            outputMessage.put(SerialNum, tokens[2].trim());
        }
        outputMessage.put(Type, type);
        if (!empty_attribute(tokens[4])) {
            outputMessage.put(ThreatContentType, unquoted_attribute(tokens[4]));
        }
        if (!empty_attribute(tokens[5])) {
            outputMessage.put(ConfigVersion, tokens[5].trim());
        }
        if (!empty_attribute(tokens[6])) {
            outputMessage.put(GenerateTime, tokens[6].trim());
        }

        if (LogTypeConfig.equalsIgnoreCase(type)) {
            // There are two fields in custom logs only and they are not in the default format.
            // But we need to parse them if they exist
            if (tokens.length == 16 || tokens.length == 18) {
                parserVersion = 61;
            } else if (tokens.length == 22 || tokens.length == 24) {
                parserVersion = 80;
            }

            if (parserVersion >= 61) {
                if (!empty_attribute(tokens[7])) {
                    outputMessage.put(HOST, tokens[7].trim());
                }
                if (!empty_attribute(tokens[8])) {
                    outputMessage.put(VirtualSystem, tokens[8].trim());
                }
                if (!empty_attribute(tokens[9])) {
                    outputMessage.put(Command, tokens[9].trim());
                }
                if (!empty_attribute(tokens[10])) {
                    outputMessage.put(Admin, tokens[10].trim());
                }
                if (!empty_attribute(tokens[11])) {
                    outputMessage.put(Client, unquoted_attribute(tokens[11]));
                }
                if (!empty_attribute(tokens[12])) {
                    outputMessage.put(Result, unquoted_attribute(tokens[12]));
                }
                if (!empty_attribute(tokens[13])) {
                    outputMessage.put(ConfigurationPath, unquoted_attribute(tokens[13]));
                }
            }
            ...
            if (parserVersion >= 70) {
                int customFieldsOffset = 0;
                if (tokens.length == 24) {
                    if (!empty_attribute(tokens[14])) {
                        outputMessage.put(BeforeChangeDetail, unquoted_attribute(tokens[14 + customFieldsOffset]));
                    }
                    if (!empty_attribute(tokens[15])) {
                        outputMessage.put(AfterChangeDetail, unquoted_attribute(tokens[15 + customFieldsOffset]));
                    }
                    customFieldsOffset = 2;
                }
                if (!empty_attribute(tokens[14 + customFieldsOffset])) {
                    outputMessage.put(Seqno, unquoted_attribute(tokens[14 + customFieldsOffset]));
                }
                if (!empty_attribute(tokens[15 + customFieldsOffset])) {
                    outputMessage.put(ActionFlags, unquoted_attribute(tokens[15 + customFieldsOffset]));
                }
                if (!empty_attribute(tokens[16 + customFieldsOffset])) {
                    outputMessage.put(DGH1, unquoted_attribute(tokens[16 + customFieldsOffset]));
                }
                if (!empty_attribute(tokens[17 + customFieldsOffset])) {
                    outputMessage.put(DGH2, unquoted_attribute(tokens[17 + customFieldsOffset]));
                }
                if (!empty_attribute(tokens[18 + customFieldsOffset])) {
                    outputMessage.put(DGH3, unquoted_attribute(tokens[18 + customFieldsOffset]));
                }
                if (!empty_attribute(tokens[19 + customFieldsOffset])) {
                    outputMessage.put(DGH4, unquoted_attribute(tokens[19 + customFieldsOffset]));
                }
                if (!empty_attribute(tokens[20 + customFieldsOffset])) {
                    outputMessage.put(VSYSName, unquoted_attribute(tokens[20 + customFieldsOffset]));
                }
                if (!empty_attribute(tokens[21 + customFieldsOffset])) {
                    outputMessage.put(DeviceName, unquoted_attribute(tokens[21 + customFieldsOffset]));
                }
            }
        } else if (LogTypeSystem.equalsIgnoreCase(type)) {
            if (tokens.length == 17) {
                parserVersion = 61;
            ...
            if (parserVersion >= 61) {
                if (!empty_attribute(tokens[7])) {
                    outputMessage.put(VirtualSystem, tokens[7].trim());
                }
                if (!empty_attribute(tokens[8])) {
                    outputMessage.put(EventId, tokens[8].trim());
                }
                if (!empty_attribute(tokens[9])) {
                    outputMessage.put(Object, tokens[9].trim());
                }

                if (!empty_attribute(tokens[12])) {
                    outputMessage.put(Module, tokens[12].trim());
                }
                if (!empty_attribute(tokens[13])) {
                    outputMessage.put(Severity, unquoted_attribute(tokens[13]));
                }
                if (!empty_attribute(tokens[14])) {
                    outputMessage.put(Description, unquoted_attribute(tokens[14]));
                }
                if (!empty_attribute(tokens[15])) {
                    outputMessage.put(Seqno, unquoted_attribute(tokens[15]));
                }
                if (!empty_attribute(tokens[16])) {
                    outputMessage.put(ActionFlags, unquoted_attribute(tokens[16]));
                }
            }

            if (parserVersion == 80) {
                if (!empty_attribute(tokens[17])) {
                    outputMessage.put(DGH1, tokens[17].trim());
                }
                if (!empty_attribute(tokens[18])) {
                    outputMessage.put(DGH2, tokens[18].trim());
                }
                if (!empty_attribute(tokens[19])) {
                    outputMessage.put(DGH3, tokens[19].trim());
                }
                if (!empty_attribute(tokens[20])) {
                    outputMessage.put(DGH4, tokens[20].trim());
                }
                if (!empty_attribute(tokens[21])) {
                    outputMessage.put(VSYSName, unquoted_attribute(tokens[21]));
                }
                if (!empty_attribute(tokens[22])) {
                    outputMessage.put(DeviceName, unquoted_attribute(tokens[22]));
                }
            }
        } else if (LogTypeThreat.equalsIgnoreCase(type)
                   || LogTypeTraffic.equalsIgnoreCase(type)) {
            if (!empty_attribute(tokens[7])) {
            ...
            if (!empty_attribute(tokens[11])) {
                outputMessage.put(Rule, unquoted_attribute(tokens[11]));
            }
            if (!empty_attribute(tokens[12])) {
                outputMessage.put(SourceUser, unquoted_attribute(tokens[12]));
            }
            if (!empty_attribute(tokens[13])) {
                outputMessage.put(DestinationUser, unquoted_attribute(tokens[13]));
            }
            if (!empty_attribute(tokens[14])) {
                outputMessage.put(Application, unquoted_attribute(tokens[14]));
            }
            if (!empty_attribute(tokens[15])) {
                outputMessage.put(VirtualSystem, unquoted_attribute(tokens[15]));
            }
            if (!empty_attribute(tokens[16])) {
                outputMessage.put(SourceZone, unquoted_attribute(tokens[16]));
            }
            if (!empty_attribute(tokens[17])) {
                outputMessage.put(DestinationZone, unquoted_attribute(tokens[17]));
            }
            if (!empty_attribute(tokens[18])) {
                outputMessage.put(InboundInterface, unquoted_attribute(tokens[18]));
            }
            if (!empty_attribute(tokens[19])) {
                outputMessage.put(OutboundInterface, unquoted_attribute(tokens[19]));
            }
            if (!empty_attribute(tokens[20])) {
                outputMessage.put(LogAction, unquoted_attribute(tokens[20]));
            }
            if (!empty_attribute(tokens[21])) {
            ...
            if (!empty_attribute(tokens[23])) {
                outputMessage.put(RepeatCount, tokens[23].trim());
            }
            if (!empty_attribute(tokens[24])) {
                outputMessage.put(SourcePort, tokens[24].trim());
            }
            if (!empty_attribute(tokens[25])) {
                outputMessage.put(DestinationPort, tokens[25].trim());
            }
            if (!empty_attribute(tokens[26])) {
                outputMessage.put(NATSourcePort, tokens[26].trim());
            }
            if (!empty_attribute(tokens[27])) {
                outputMessage.put(NATDestinationPort, tokens[27].trim());
            }
            if (!empty_attribute(tokens[28])) {
                outputMessage.put(Flags, tokens[28].trim());
            }
            if (!empty_attribute(tokens[29])) {
                outputMessage.put(IPProtocol, unquoted_attribute(tokens[29]));
            }
            if (!empty_attribute(tokens[30])) {
                outputMessage.put(Action, unquoted_attribute(tokens[30]));
            }

            if (LogTypeThreat.equalsIgnoreCase(type)) {
                int p1Offset = 0;
                if (tokens.length == 45) {
                    parserVersion = 60;
                } else if (tokens.length == 53) {
                    parserVersion = 61;
                ...
                if (!empty_attribute(tokens[31])) {
                    String rawUrl = unquoted_attribute(tokens[31]);
                    outputMessage.put(URL, rawUrl);
                    try {
                        URL url = new URL(rawUrl);
                        outputMessage.put(HOST, url.getHost());
                    } catch (MalformedURLException e) {
                        _LOG.debug("Malformed URL: " + rawUrl, e);
                    }
                }
                if (!empty_attribute(tokens[32])) {
                    outputMessage.put(ThreatID, tokens[32].trim());
                }
                if (!empty_attribute(tokens[33])) {
                    outputMessage.put(Category, unquoted_attribute(tokens[33]));
                }
                if (!empty_attribute(tokens[34])) {
                    outputMessage.put(Severity, unquoted_attribute(tokens[34]));
                }
                if (!empty_attribute(tokens[35])) {
                    outputMessage.put(Direction, unquoted_attribute(tokens[35]));
                }
                if (!empty_attribute(tokens[36])) {
                    outputMessage.put(Seqno, tokens[36].trim());
                }
                if (!empty_attribute(tokens[37])) {
                    outputMessage.put(ActionFlags, unquoted_attribute(tokens[37]));
                }
                if (!empty_attribute(tokens[38])) {
                    outputMessage.put(SourceLocation, unquoted_attribute(tokens[38]));
                }
                if (!empty_attribute(tokens[39])) {
                    outputMessage.put(DestinationLocation, unquoted_attribute(tokens[39]));
                }
                if (!empty_attribute(tokens[41])) {
                    outputMessage.put(ContentType, unquoted_attribute(tokens[41]));
                }
                if (!empty_attribute(tokens[42])) {
                    outputMessage.put(PCAPID, tokens[42].trim());
                }
                if (!empty_attribute(tokens[43])) {
                    outputMessage.put(WFFileDigest, unquoted_attribute(tokens[43]));
                }
                if (!empty_attribute(tokens[44])) {
                    outputMessage.put(WFCloud, unquoted_attribute(tokens[44]));
                }
                if (parserVersion >= 61) {
                    if (!empty_attribute(tokens[(45 + p1Offset)])) {
                        outputMessage.put(UserAgent, unquoted_attribute(tokens[(45 + p1Offset)]));
                    }
                    if (!empty_attribute(tokens[(46 + p1Offset)])) {
                        outputMessage.put(WFFileType, unquoted_attribute(tokens[(46 + p1Offset)]));
                    }
                    if (!empty_attribute(tokens[(47 + p1Offset)])) {
                        outputMessage.put(XForwardedFor, unquoted_attribute(tokens[(47 + p1Offset)]));
                    }
                    if (!empty_attribute(tokens[(48 + p1Offset)])) {
                        outputMessage.put(Referer, unquoted_attribute(tokens[(48 + p1Offset)]));
                    }
                    if (!empty_attribute(tokens[(49 + p1Offset)])) {
                        outputMessage.put(WFSender, unquoted_attribute(tokens[(49 + p1Offset)]));
                    }
                    if (!empty_attribute(tokens[(50 + p1Offset)])) {
                        outputMessage.put(WFSubject, unquoted_attribute(tokens[(50 + p1Offset)]));
                    }
                    if (!empty_attribute(tokens[(51 + p1Offset)])) {
                        outputMessage.put(WFRecipient, unquoted_attribute(tokens[(51 + p1Offset)]));
                    }
                    if (!empty_attribute(tokens[(52 + p1Offset)])) {
                        outputMessage.put(WFReportID, unquoted_attribute(tokens[(52 + p1Offset)]));
                    }
                }
                if (parserVersion >= 70) {
                    if (!empty_attribute(tokens[45])) {
                        outputMessage.put(URLIndex, tokens[45].trim());
                    }
                    if (!empty_attribute(tokens[54])) {
                        outputMessage.put(DGH1, tokens[54].trim());
                    }
                    if (!empty_attribute(tokens[55])) {
                        outputMessage.put(DGH2, tokens[55].trim());
                    }
                    if (!empty_attribute(tokens[56])) {
                        outputMessage.put(DGH3, tokens[56].trim());
                    }
                    if (!empty_attribute(tokens[57])) {
                        outputMessage.put(DGH4, tokens[57].trim());
                    }
                    if (!empty_attribute(tokens[58])) {
                        outputMessage.put(VSYSName, unquoted_attribute(tokens[58]));
                    }
                    if (!empty_attribute(tokens[59])) {
                        outputMessage.put(DeviceName, unquoted_attribute(tokens[59]));
                    }
                }
                if (parserVersion >= 80) {
                    if (!empty_attribute(tokens[61])) {
                        outputMessage.put(SourceVmUuid, tokens[61].trim());
                    }
                    if (!empty_attribute(tokens[62])) {
                        outputMessage.put(DestinationVmUuid, tokens[62].trim());
                    }
                    if (!empty_attribute(tokens[63])) {
                        outputMessage.put(HTTPMethod, tokens[63].trim());
                    }
                    if (!empty_attribute(tokens[64])) {
                        outputMessage.put(TunnelId, tokens[64].trim());
                    }
                    if (!empty_attribute(tokens[65])) {
                        outputMessage.put(MonitorTag, tokens[65].trim());
                    }
                    ...
                    if (!empty_attribute(tokens[68])) {
                        outputMessage.put(TunnelType, tokens[68].trim());
                    }
                    if (!empty_attribute(tokens[69])) {
                        outputMessage.put(ThreatCategory, tokens[69].trim());
                    }
                    if (!empty_attribute(tokens[70])) {
                        outputMessage.put(ContentVersion, tokens[70].trim());
                    }
                }
            } else if (LogTypeTraffic.equalsIgnoreCase(type)) {
                if (tokens.length == 46) {
                    parserVersion = 60;
                } else if (tokens.length == 47) {
                    parserVersion = 61;
                ...
                if (!empty_attribute(tokens[37])) {
                    outputMessage.put(Category, unquoted_attribute(tokens[37]));
                }
                if (!empty_attribute(tokens[39])) {
                    outputMessage.put(Seqno, tokens[39].trim());
                }
                if (!empty_attribute(tokens[40])) {
                    outputMessage.put(ActionFlags, unquoted_attribute(tokens[40]));
                }
                if (!empty_attribute(tokens[41])) {
                    outputMessage.put(SourceLocation, unquoted_attribute(tokens[41]));
                }
                if (!empty_attribute(tokens[42])) {
                ...
                if (parserVersion >= 61) {
                    if (!empty_attribute(tokens[46])) {
                        outputMessage.put(EndReason, unquoted_attribute(tokens[46]));
                    }
                }
                if (parserVersion >= 70) {
                    if (!empty_attribute(tokens[47])) {
                        outputMessage.put(DGH1, tokens[47].trim());
                    }
                    if (!empty_attribute(tokens[48])) {
                        outputMessage.put(DGH2, tokens[48].trim());
                    }
                    if (!empty_attribute(tokens[49])) {
                        outputMessage.put(DGH3, tokens[49].trim());
                    }
                    if (!empty_attribute(tokens[50])) {
                        outputMessage.put(DGH4, tokens[50].trim());
                    }
                    if (!empty_attribute(tokens[51])) {
                        outputMessage.put(VSYSName, unquoted_attribute(tokens[51]));
                    }
                    if (!empty_attribute(tokens[52])) {
                        outputMessage.put(DeviceName, unquoted_attribute(tokens[52]));
                    }
                    if (!empty_attribute(tokens[53])) {
                        outputMessage.put(ActionSource, unquoted_attribute(tokens[53]));
                    }
                }
                if (parserVersion >= 80) {
                    if (!empty_attribute(tokens[54])) {
                        outputMessage.put(SourceVmUuid, tokens[54].trim());
                    }
                    if (!empty_attribute(tokens[55])) {
                        outputMessage.put(DestinationVmUuid, tokens[55].trim());
                    }
                    if (!empty_attribute(tokens[56])) {
                        outputMessage.put(TunnelId, tokens[56].trim());
                    }
                    if (!empty_attribute(tokens[57])) {
                        outputMessage.put(MonitorTag, tokens[57].trim());
                    }
                    if (!empty_attribute(tokens[58])) {
                    ...
                    if (!empty_attribute(tokens[60])) {
                        outputMessage.put(TunnelType, tokens[60].trim());
                    }
                }
            }
        }
        outputMessage.put(ParserVersion, parserVersion);
        if (parserVersion == 0) {
