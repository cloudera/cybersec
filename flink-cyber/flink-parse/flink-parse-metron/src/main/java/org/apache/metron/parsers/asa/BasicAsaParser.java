...
public class BasicAsaParser extends BasicParser {
    ...
    private final Map<String, Grok> grokers = new HashMap<String, Grok>(patternMap.size());
    ...
    public void init() {
        ...
        for (Entry<String, String> pattern : patternMap.entrySet()) {
            try {
                ...
                LOG.error("[Metron] Failed to load grok pattern {} for ASA tag {}", pattern.getValue(),
                      pattern.getKey());
            }
        }
        ...
        try {
            ...
            if (asaGrok == null) {
                ...
                if (!messageMatch.isNull()) {
                    ...
                    String srcIp = (String) messageJson.get("src_ip");
                    if (srcIp != null) {
                        metronJson.put(Constants.Fields.SRC_ADDR.getName(), srcIp);
                    }

                    Integer srcPort = (Integer) messageJson.get("src_port");
                    if (srcPort != null) {
                        metronJson.put(Constants.Fields.SRC_PORT.getName(), srcPort);
                    }

                    String dstIp = (String) messageJson.get("dst_ip");
                    if (dstIp != null) {
                        metronJson.put(Constants.Fields.DST_ADDR.getName(), dstIp);
                    }

                    Integer dstPort = (Integer) messageJson.get("dst_port");
                    if (dstPort != null) {
                        metronJson.put(Constants.Fields.DST_PORT.getName(), dstPort);
                    }
