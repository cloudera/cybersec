...
public class BasicBroParser extends BasicParser {
    ...
    private final JSONCleaner cleaner = new JSONCleaner();
    ...
    public List<JSONObject> parse(byte[] msg) {
        ...
        try {
            ...
            boolean ipSrcReplaced =
                  replaceKey(payload, Constants.Fields.SRC_ADDR.getName(), new String[] {"source_ip", "id.orig_h"});
            if (!ipSrcReplaced) {
                replaceKeyArray(payload, Constants.Fields.SRC_ADDR.getName(), new String[] {"tx_hosts"});
            }

            boolean ipDstReplaced =
                  replaceKey(payload, Constants.Fields.DST_ADDR.getName(), new String[] {"dest_ip", "id.resp_h"});
            if (!ipDstReplaced) {
